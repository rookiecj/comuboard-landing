  ...
  return (
    <div className="flex w-[320px] sm:w-[380px] flex-shrink-0 gap-4 rounded-2xl border border-slate-200 dark:border-slate-700/50 bg-white dark:bg-slate-800/50 p-6 backdrop-blur-md transition-all hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:border-slate-300 dark:hover:border-slate-600 shadow-sm dark:shadow-none">
      {community.logoUrl ? (
        ...
        <div
          className={`flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-xl text-lg font-bold text-white shadow-inner ring-1 ring-slate-900/5 dark:ring-white/10 ${getInitialBgColor(community.name)}`}
        >
...
export function Showcase() {
  ...
  useEffect(() => {
    ...
    async function fetchCommunities() {
      try {
        const res = await fetch(`${API_BASE}/api/service-landing/public-communities?limit=6`);
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 mb-20 flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
        ...
        >
          <h2 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-4xl lg:text-5xl">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-500">커뮤니티</span>
          </h2>
          <p className="mt-6 text-lg sm:text-xl text-slate-600 dark:text-slate-400 leading-relaxed">
            ...
            다양한 관심사를 가진 사람들이 모여 이미 ComuBoard에서 
            폭넓은 이야기와 지식을 쌓아가고 있습니다.
          </p>
        </motion.div>
      </div>
        ...
        {loading ? (
          <div className="flex gap-6 px-6 animate-pulse">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="flex flex-shrink-0 w-[320px] sm:w-[380px] gap-4 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800/30 p-6">
          ...
          >
            {scrollItems.map((community, i) => (
              <CommunityCard key={`${community.id}-${i}`} community={community} />
            ))}
          </motion.div>
        )}
      </div>
