        ...
        <ul className="mt-5 space-y-2" aria-label="담은 게시판 목록">
          {addedBoards.length === 0 ? (
            <li className="rounded-xl border border-dashed border-slate-200 px-4 py-6 text-center text-sm text-slate-400 dark:border-slate-700 dark:text-slate-500">
              {/* 모바일: 공간이 위·게시판 목록이 아래 / 데스크톱: 팔레트가 좌측 */}
  ...
  return (
    <section className="relative overflow-hidden bg-slate-50 py-24 transition-colors duration-300 dark:bg-slate-950 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        {/* 조립 인터랙션: 데스크톱=좌 팔레트/우 공간, 모바일=공간 먼저→게시판 목록(4개+스크롤) */}
        <div className="mt-16 grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-12">
          {/* 게시판 팔레트 — 모바일에선 공간 아래(order-2), 데스크톱 좌측(order-1) */}
          <div
          ...
          >
            ...
            {/* 모바일: 4개 노출 + 5번째 살짝 보이게(스크롤 힌트) 후 세로 스크롤 / 데스크톱: 2열 전체 */}
            <div className="relative">
              <div className="grid max-h-[25rem] grid-cols-1 gap-3 overflow-y-auto pr-1 sm:max-h-none sm:grid-cols-2 sm:overflow-visible sm:pr-0">
                {/* 고정 3종 (AC2) */}
              ...
              {/* 모바일: 하단 페이드로 스크롤 여지 표시 */}
              <div
                className="pointer-events-none absolute inset-x-0 bottom-0 h-10 bg-gradient-to-t from-slate-50 to-transparent dark:from-slate-950 sm:hidden"
                aria-hidden
              />
            </div>

            {/* "더 많은 게시판" 넛지 (AC4) — 스크롤 영역 밖에 항상 노출 */}
            <a
              href={APP_ROUTES.boards}
              className="group mt-3 flex w-full items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-slate-300 px-4 py-4 text-center text-sm font-semibold text-slate-500 transition hover:border-brand-400 hover:text-brand-600 dark:border-slate-700 dark:text-slate-400 dark:hover:border-brand-400/60 dark:hover:text-brand-300"
            >
              이 외에도 더 있어요 · 전체 둘러보기
          ...
          {/* 우리 공간 프레임 — 모바일 먼저(order-1), 데스크톱 우측(order-2) */}
